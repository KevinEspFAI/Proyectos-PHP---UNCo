...
/** 2) Mostrar la info completa del mes con mayor costo de compra
 * @param array $costos
 * @param array $prodMayorCosto
 */
function opcionDos($costos, $prodMayorCosto) {
    //int $i, $numeroMayorMes
    //float $costomMayorMes
    ...
    for ($i = 1; $i < count($costos); $i++) {
        if ($costos[$i] > $costoMayorMes) {
            ...
            $numeroMayorMes = $i; //Se actualiza el índice del mes con mayor costo que Enero
        }
    }
    
    //Se invoca al traductor para que nos dé el nombre del mes de mayor costo
    $nombreMayorMes = intToString($numeroMayorMes);
    
    // Se muestra la información
    echo "Mostrando información del mes con mayor costo: (".$nombreMayorMes.")\n";
   
    echo "El juguete con mayor costo: ".$prodMayorCosto[$numeroMayorMes]["prod"]."\n";
    echo "Cantidad comprados: ".$prodMayorCosto[$numeroMayorMes]["cantProd"]."\n";
    echo "Precio unitario: ".$prodMayorCosto[$numeroMayorMes]["precioProd"]."\n";
    echo "Costo acumulado por compras del mes de ".$nombreMayorMes. ": ".$costos[$numeroMayorMes];
    
}


/** 3) Imprime cuál fue el 1° mes que supera el monto total de compra (ingresado por el usuario desde el prog. principal)
 * @param int $monto
 * @param string $nombreMes2
 * @param array $costos
 */
function opcionTres ($monto, $costos, $prodMayorCosto) {
    //int $i
    //boolean $encontrado
    //string $nombreMes2
    ...
    while ($i < count($costos) && $encontrado == false) {
        ...
        if ($costos[$i] > $monto) {         //si el valor el costo del mes es mayor a monto limite, imprimir ése mes
            ...
            echo "El primer mes que supera el monto límite establecido "."($".$monto.") "." es el mes de: ".$nombreMes2."\n";
            
            // Se muestra la información
            echo "\nMostrando información del mes de (".$nombreMes2.")\n";
            echo "El juguete con mayor costo: ".$prodMayorCosto[$i]["prod"]."\n";
            echo "Cantidad comprados: ".$prodMayorCosto[$i]["cantProd"]."\n";
            echo "Precio unitario: ".$prodMayorCosto[$i]["precioProd"]."\n";
            echo "Costo acumulado por compras del mes de ".$nombreMes2. ": ".$costos[$i];
...
function opcionCuatro ($costos, $prodMayorCosto){
    ...
    // Se muestra la información
    echo "Mostrando información del mes de (".$nombreMes.")\n";
    echo "El juguete con mayor costo: ".$prodMayorCosto[$numeroMes]["prod"]."\n";
    echo "Cantidad comprados: ".$prodMayorCosto[$numeroMes]["cantProd"]."\n";
    echo "Precio unitario: ".$prodMayorCosto[$numeroMes]["precioProd"]."\n";
    echo "Costo acumulado por compras del mes de ".$nombreMes. ": ".$costos[$numeroMes];
}
...
//Se inicializa como false el ingreso de monto máximo de compra
$ingresoDeMonto = false;
...
Do{
    ...
    if ($eleccion == 1){
        ...
        $prodMayorCosto = opcionUnoD($costos, $prodMayorCosto, $numeroMesProg, $datosProdProg);
            ...
            opcionDos($costos, $prodMayorCosto);
            
    } elseif ($eleccion == 3){
        
        //si no hay establecido un limite aun, se lo pide
        if ($ingresoDeMonto == false) {
            echo "No ha ingresado monto límite aún.\n";
            echo "Ingrese monto total límite: $";
            $montoLimite = trim(fgets(STDIN));
            
            $ingresoDeMonto = true;
        }
        
        //se invoca finalmente a la opcionTres
        opcionTres($montoLimite, $costos, $prodMayorCosto);
        
    } elseif ($eleccion == 4){
       
        opcionCuatro($costos, $prodMayorCosto);
